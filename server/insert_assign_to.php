<?php 
    ...
    $assign_date = (new DateTime())->format('Y-m-d');
    $comment = $data['assignment_comment'];
     ...
     $stmt = $con->prepare ("INSERT INTO `assign_request` (`request_id`,`assign_to_id`,`assign_to_username`,`assign_to_user_email`, `assign_to_department`, `assign_by_id`, `assign_by_username`, `assign_by_user_email`, `assign_by_user_department`, `assign_date`, `comment`) 
     VALUES ('".$request_id."','".$request_registrar."','".$assignedTo['user_name_ar']."','".$assignedTo['user_email']."', '".$assignedToDepartment['name']."', '".$assigned_by."', '".$assignedBy['user_name_ar']."', '".$assignedBy['user_email']."', '".$assignedByDepartment['name']."', '".$assign_date."', '".$comment."')");
     $query = $stmt->execute();

     $stmt2 = $con->prepare ("UPDATE `request_information` SET `assigned_to` = '".$request_registrar."', `assigned_date` = '".$assign_date."', `assigned_department` = '".$assignedToDepartment['name']."'  WHERE `request_id` = '".$request_id."'");
